...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    loss = torch.sum(-scores[torch.arange(N), y] + torch.log(torch.sum(torch.exp(scores), dim=1)))
    loss /= N
    ...
    # dL / ds, (N, C)
    grad_scores = torch.exp(scores) / torch.sum(torch.exp(scores), dim=1).view(-1, 1)
    grad_scores[torch.arange(N), y] -= 1 # (N, C)
    grad_scores
    
    # N, C
    grad_b2 = torch.sum(grad_scores, dim=0) / N
    grads['b2'] = grad_b2

    # H, C
    grad_W2 = (h1.T @ grad_scores) / N + 2 * reg * W2
    grads['W2'] = grad_W2

    # N, H
    grad_relu = grad_scores @ W2.T
    grad_relu[h1 == 0] = 0

    grad_b1 = torch.sum(grad_relu, dim=0) / N
    grads['b1'] = grad_b1

    grad_W1 = (X.T @ grad_relu) / N + 2 * reg * W1
    grads['W1'] = grad_W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths = [0.00001, 0.0001, 0.001, 0.01]
    learning_rate_decays = []
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
