...
def svm_loss_vectorized(
    ...
    D, C = W.shape
    scores = X @ W
    difference = scores - scores[torch.arange(N), y][:, None] # (N, C) -> num_train_batch, num_classes
    # dW is (D, C)
    difference[torch.arange(N), y] = -1
    difference += 1
    mask = difference <= 0
    difference[mask] = 0
    loss += torch.sum(difference) / N
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    dW = X.T @ difference
    one_hot = torch.zeros(N, C, dtype=dW.dtype)
    one_hot[torch.arange(N), y] = torch.sum(difference, dim=1)
    dW -= X.T @ one_hot
    dW /= N
    dW += 2 * reg * W
    #############################################################################
...
def softmax_loss_naive(
    ...
    loss = torch.sum(-logits[torch.arange(N), y] + torch.log(torch.sum(torch.exp(logits), dim=1))) / N + reg * torch.sum(W * W)

    for i in range(N):
        logit = W.T @ X[i]
        normalizer = 1 / torch.sum(torch.exp(logit))
        dW[:, y[i]] -= X[i]
        for j in range(C):
            dW[:, j] += normalizer * torch.exp(logit[j]) * X[i]
...
def softmax_loss_vectorized(
    ...
    dW -= X.T @ one_hot
    post_softmax = torch.exp(logits) / torch.sum(torch.exp(logits), dim=1)[:, None]
    dW += X.T @ post_softmax
    dW /= N
    dW += 2 * reg * W
    #############################################################################
