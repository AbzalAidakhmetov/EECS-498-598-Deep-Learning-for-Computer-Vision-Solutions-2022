...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dw = x.reshape(x.shape[0], -1).T @ dout   # N, M @ N @ D
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        self.W1 = torch.randn(size=(input_dim, hidden_dim), dtype=dtype, device=device) * torch.tensor(weight_scale)
        self.b1 = torch.zeros(size=(hidden_dim,), dtype=dtype, device=device) 
        self.W2 = torch.randn(size=(hidden_dim, num_classes), dtype=dtype, device=device) * torch.tensor(weight_scale)
        self.b2 = torch.zeros(size=(num_classes,), dtype=dtype, device=device)
        self.reg = reg
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out_lrelu, cache_lrelu = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, cache_l = Linear.forward(out_lrelu, self.params['W2'], self.params['b2'])
        ...
        loss += self.reg * torch.sum(self.params['W2'] ** 2) + self.reg * torch.sum(self.params['W1'] ** 2)
        dAfter_Relu, dW2, db2  = Linear.backward(loss_grad, cache_l)
        
        grads['W2'] = dW2 + 2 * self.reg * self.params['W2']
        grads['b2'] = db2

        _, dW1, db1 = Linear_ReLU.backward(dAfter_Relu, cache_lrelu)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            if i == 0:
                self.params[f'W{i + 1}'] = torch.randn(size=(input_dim, hidden_dims[i]), dtype=dtype, device=device) * weight_scale
            else:
                self.params[f'W{i + 1}'] = torch.randn(size=(hidden_dims[i - 1], hidden_dims[i]), dtype=dtype, device=device) * weight_scale
            
            self.params[f'b{i + 1}'] = torch.zeros(size=(hidden_dims[i], ), dtype=dtype, device=device)

        self.params[f'W{self.num_layers}'] = torch.randn(size=(hidden_dims[-1], num_classes), dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for j in range(self.num_layers - 1):
            if j == 0:
                intermediate[f'out_{j + 1}'], intermediate[f'cache_{j + 1}'] =  Linear_ReLU.forward(X, self.params[f'W{j + 1}'], self.params[f'b{j + 1}'])
                if self.use_dropout:
                    intermediate[f'out_drop_{j + 1}'], intermediate[f'cache_drop_{j + 1}'] = Dropout.forward(intermediate[f'out_{j + 1}'], self.dropout_param)
                continue
            
            if self.use_dropout:
                  intermediate[f'out_{j + 1}'], intermediate[f'cache_{j + 1}'] =  Linear_ReLU.forward(intermediate[f'out_drop_{j}'], self.params[f'W{j + 1}'], self.params[f'b{j + 1}'])
                  intermediate[f'out_drop_{j + 1}'], intermediate[f'cache_drop_{j + 1}'] = Dropout.forward(intermediate[f'out_{j + 1}'], self.dropout_param)
                  
            else:
                  intermediate[f'out_{j + 1}'], intermediate[f'cache_{j + 1}'] =  Linear_ReLU.forward(intermediate[f'out_{j}'], self.params[f'W{j + 1}'], self.params[f'b{j + 1}'])

        if self.use_dropout:
              intermediate[f'out_{self.num_layers}'], intermediate[f'cache_{self.num_layers}'] = Linear.forward(intermediate[f'out_drop_{self.num_layers - 1}'], self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'] )
        else:
              intermediate[f'out_{self.num_layers}'], intermediate[f'cache_{self.num_layers}'] = Linear.forward(intermediate[f'out_{self.num_layers - 1}'], self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'] )
        scores = intermediate[f'out_{self.num_layers}']
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        upstream_grad, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(grad_soft, intermediate[f'cache_{self.num_layers}'])

        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']
        loss += 0.5 * self.reg * torch.sum(self.params[f'W{self.num_layers}'] ** 2)

        for k in range(self.num_layers - 1, 0, -1):
            if self.dropout_param:
                upstream_grad = Dropout.backward(upstream_grad, intermediate[f'cache_drop_{k}'])
            upstream_grad, grads[f'W{k}'], grads[f'b{k}'] = Linear_ReLU.backward(upstream_grad, intermediate[f'cache_{k}'])
            
            grads[f'W{k}'] += self.reg * (self.params[f'W{k}'])
            loss += 0.5 * self.reg * torch.sum(self.params[f'W{k}'] ** 2)
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, lr_decay=0.5, num_epochs=20, batch_size=256, optim_config=config)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = (2 / 100) ** 0.5   # Experiment with this!
...
class Dropout(object):
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            ###########################################################
            # Replace "pass" statement with your code
            grad_dropout = torch.zeros(*mask.shape)
            grad_dropout[mask] = 1 / (1 - dropout_param['p'])
            dx = dout * grad_dropout
            ###########################################################
            #                     END OF YOUR CODE                    #
            ###########################################################
        elif mode == 'test':
