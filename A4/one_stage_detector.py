...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        """
        NOTE: two separate conv + relu layers for cls and box stems, also initalize with nn.init
        """
        for channel in stem_channels:
            conv_cls = nn.Conv2d(cur_channel, channel, 3, 1, 1)
            nn.init.normal_(conv_cls.weight, 0, 0.01)
            nn.init.constant_(conv_cls.bias, 0)
            stem_cls.extend([conv_cls, nn.ReLU()])

            conv_box = nn.Conv2d(cur_channel, channel, 3, 1, 1)
        ...
        # Replace "pass" statement with your code
        self.pred_cls = torch.nn.Conv2d(stem_channels[-1], num_classes, 3, 1, 1)
        nn.init.normal_(self.pred_cls.weight, 0, 0.01)
        nn.init.constant_(self.pred_cls.bias, 0)
        self.pred_box = torch.nn.Conv2d(stem_channels[-1], 4, 3, 1, 1)
        nn.init.normal_(self.pred_box.weight, 0, 0.01)
        nn.init.constant_(self.pred_box.bias, 0)
        self.pred_ctr = torch.nn.Conv2d(stem_channels[-1], 1, 3, 1, 1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level, feat in feats_per_fpn_level.items():
            B, C, H, W = feat.shape
            stem_class = self.stem_cls(feat)
            
            stem_box_ctr_feat = self.stem_box(feat)

            class_logits[level] = self.pred_cls(stem_class).reshape(B, self.num_classes, -1).permute(0, 2, 1)
            boxreg_deltas[level] = self.pred_box(stem_box_ctr_feat).reshape(B, 4, -1).permute(0, 2, 1)
            centerness_logits[level] = self.pred_ctr(stem_box_ctr_feat).reshape(B, 1, -1).permute(0, 2, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.stack(
        [locations[:, 0] - gt_boxes[:, 0], locations[:, 1] - gt_boxes[:, 1], gt_boxes[:, 2] - locations[:, 0], gt_boxes[:, 3] - locations[:, 1]], dim=1
    )
    deltas /= stride
    deltas[gt_boxes.min(dim=1).values < 0] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    deltas *= stride
    output_boxes = torch.zeros_like(deltas) # this is to make sure that output boxes will be in the same device
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    ##########################################################################
    # NOTE: my original implementation
    # centerness = torch.sqrt(deltas[:, [0, 2]].min(dim=1).values / deltas[:, [0, 2]].max(dim=1).values * deltas[:, [1, 3]].min(dim=1).values / deltas[:, [1, 3]].max(dim=1).values)
    # centerness[deltas.min(dim=1).values < 0] = -1
    # Below is the AI's implementation
    ##########################################################################

    centerness = None
    # Replace "pass" statement with your code
    
    # First identify valid (non-background) boxes
    is_bg = deltas.min(dim=1).values < 0
    
    # Initialize centerness tensor
    centerness = torch.zeros(deltas.shape[0], device=deltas.device)
    
    # Only calculate centerness for non-background boxes
    if (~is_bg).sum() > 0:
        # Get min and max values for x and y directions
        left_right_min = deltas[~is_bg][:, [0, 2]].min(dim=1).values
        left_right_max = deltas[~is_bg][:, [0, 2]].max(dim=1).values
        top_bottom_min = deltas[~is_bg][:, [1, 3]].min(dim=1).values
        top_bottom_max = deltas[~is_bg][:, [1, 3]].max(dim=1).values
        
        # Avoid division by zero with a small epsilon
        epsilon = 1e-8
        centerness_fg = torch.sqrt(
            (left_right_min / (left_right_max + epsilon)) * 
            (top_bottom_min / (top_bottom_max + epsilon))
        )
        
        # Assign calculated centerness to non-background positions
        centerness[~is_bg] = centerness_fg
    
    # Set background positions to -1
    centerness[is_bg] = -1
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for i in range(M):
            ...
            for level, locations in locations_per_fpn_level.items():
                    cur_d[level] = fcos_get_deltas_from_locations(locations, matched_gt_boxes[i][level], self.backbone.fpn_strides[level])
            matched_gt_deltas.append(cur_d)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        # print(matched_gt_boxes.shape, matched_gt_deltas.shape, pred_cls_logits.shape)
       
        # NOTE: Below is the AI's implementation
        ######################################################################
        B, N, _ = matched_gt_boxes.shape
        
        # 1) extract & clamp class indices to [-1, num_classes-1]
        class_idxs = matched_gt_boxes[..., -1].long().clamp(min=-1, max=self.num_classes-1)
        #    shape: (16, 1029)

        # 2) build a broadcastable range vector [0,1,…,19] in the last dim
        #    shape: (1, 1, 20) here, since class_idxs.dim() == 2
        shape = [1] * class_idxs.dim() + [self.num_classes]
        class_range = torch.arange(self.num_classes, device=class_idxs.device).view(shape)
        #    shape: (1, 1, 20)

        # 3) compare & cast
        #    class_idxs.unsqueeze(-1): (16, 1029, 1)
        #    == class_range:           (16, 1029, 20)
        gt_classes = (class_idxs.unsqueeze(-1) == class_range).float()
        gt_classes[class_idxs == -1] = 0.0
        ######################################################################
        loss_cls = sigmoid_focal_loss(pred_cls_logits, targets=gt_classes)
        ...
        # print(pred_ctr_logits.shape, matched_gt_deltas.shape, gt_centerness.shape)

        valid_cent = (gt_centerness >= 0)           # (B, N, 1) boolean
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_scores = torch.max(level_pred_scores, dim=1).values # (N, 1)
            
            # Step 2:
            # Replace "pass" statement with your code
            good_predictions = level_pred_scores > test_score_thresh
            
            # NOTE: Forgot to handle the case where all predictions are below threshold, so I added this
            # Handle case where all predictions are below threshold
            if good_predictions.sum() == 0:
                # Add empty tensors with the correct shapes and devices
                pred_boxes_all_levels.append(torch.zeros((0, 4), device=level_pred_scores.device))
                pred_classes_all_levels.append(torch.zeros((0,), dtype=torch.long, device=level_pred_scores.device))
                pred_scores_all_levels.append(torch.zeros((0,), device=level_pred_scores.device))
                continue
                
            level_pred_classes = level_pred_classes[good_predictions]
            level_pred_scores = level_pred_scores[good_predictions]
            # print(level_pred_scores.sort(descending=True))
            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas[good_predictions], level_locations[good_predictions], self.backbone.fpn_strides[level_name])
            ...
            level_pred_boxes[:, 2] = level_pred_boxes[:, 2].clip(max=W - 1)
            level_pred_boxes[:, 3] = level_pred_boxes[:, 3].clip(max=H - 1)
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
        ...
        # NOTE: Again, forgot to handle the case where there are no valid predictions across all levels, so I added this
        # Handle case when there are no valid predictions across all levels
        if pred_boxes_all_levels.shape[0] == 0:
            return (
                pred_boxes_all_levels,
                pred_classes_all_levels,
                pred_scores_all_levels,
        ...
        # print(pred_boxes_all_levels.shape, pred_classes_all_levels.shape, pred_scores_all_levels.shape)
