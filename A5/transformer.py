...
def scaled_dot_product_two_loop_single(
    ...
    for q in range(K):
        ...
        for k in range(K):
            cur_key = key[k, :]
            cur_raw.append(cur_query.dot(cur_key))
        cur_raw = torch.tensor(cur_raw) / M ** 0.5 # (K, ) for current query raw scores to which of the keys are more important than the other ones
        cur_attn = torch.softmax(cur_raw, 0)
        out[q] = cur_attn @ value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    for q in range(K):
        cur_queries = query[:, q, :] # (N, M)
        cur_raw = []
        for k in range(K):
            cur_keys = key[:, k, :] # (N, M)
            cur_raw.append(torch.sum(cur_queries * cur_keys, dim=1)) # (N, ) each N represent the raw alignment score for particuar key
        cur_raw = torch.stack(cur_raw, dim=1) / M ** 0.5 # (N, K)
        cur_attn = torch.softmax(cur_raw, dim=1) # (N, K)
        out[:, q, :] = torch.bmm(cur_attn.unsqueeze(1), value).squeeze()  # (N, 1, K) @ (N, K, M) -> (N, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    raw_scores = torch.bmm(query, key.permute(0, 2, 1)) / M ** 0.5 # N, K, K
    ...
    if mask is not None:
        ...
        raw_scores = torch.masked_fill(raw_scores, mask, -1e9)
    # Replace "pass" statement with your code
    weights_softmax = torch.softmax(raw_scores, dim=2)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for i in range(num_heads):
            self.module_list.append(SelfAttention(dim_in, dim_out, dim_out))
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        multihead_list = [] # each element is (N, K, dim_out)
        for model in self.module_list:
            y_cur = model(query, key, value, mask)
            multihead_list.append(y_cur)
        multihead_output = torch.concat(multihead_list, dim=2) # (N, K, num_head * dim_out)
        y = self.linear(multihead_output)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        if x.dim() == 2:
            # if we have only (N, K), we treat the last dim as the embed_dim
            x = x[:, None, :]
        
        mean = torch.mean(x, dim=(2)) # (N, K)
        std = torch.sum((x - mean[:, :, None]) ** 2 / (x.shape[2]), dim=(2)) ** 0.5
        
        y = self.gamma * ((x - mean[:, :, None]) / (std[:, :, None] + self.epsilon)) + self.beta
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.first_linear = nn.Linear(inp_dim, hidden_dim_feedforward)
        c = (6/(inp_dim + hidden_dim_feedforward)) ** 0.5
        nn.init.uniform_(self.first_linear.weight, -c, c)
        self.second_linear = nn.Linear(hidden_dim_feedforward, inp_dim)
        nn.init.uniform_(self.second_linear.weight, -c, c)
    ...
    def forward(self, x):
        ...
        y = self.second_linear(nn.ReLU()(self.first_linear(x)))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # out2 = self.dropout_layer(self.layernorm_1(out1 + x)) 
        # out3 = self.feedforward(out2)

        # y = out = self.dropout_layer(self.layernorm_2(out3 + out2)) 
        # After multi-head attention:
        out1 = self.multihead_attn(x, x, x)  
        out2 = self.layernorm_1(x + self.dropout_layer(out1))
        # After feed-forward:
        out3 = self.feedforward(out2)
        y = self.layernorm_2(out2 + self.dropout_layer(out3))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    one_layer_mask = torch.ones(K, K, device=seq.device)
    mask[torch.arange(N), :, :] = torch.triu(one_layer_mask, diagonal=1)
    mask = mask.bool()
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        y = out =  self.dropout(self.norm3(out5 + out4))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    seq = torch.arange(K)[:, None] / K
    y = seq.repeat((1, M))[None, ...]
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # NOTE: This was done by chatgpt.
     ...
     # 1) Allocate (1, K, M) float32
    y = torch.zeros(1, K, M, dtype=torch.float32)

    # 2) Positions 0..K-1
    p = torch.arange(K, dtype=torch.float32)

    # 3) Fill even dims with sin, odd dims with cos
    y[0, :, 0::2] = torch.sin(p).unsqueeze(1)  # shape (K,1) → broadcast over evens
    y[0, :, 1::2] = torch.cos(p).unsqueeze(1)  # likewise for odds
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        a_emb = self.emb_layer(ans_b) # shape (N, K, M)
        ...
        mask = get_subsequent_mask(ans_b[:, :-1]) # all except the last, which corresponds with shifted to right outputs
        dec_out = self.decoder(a_emb_inp, enc_out, mask).reshape(-1, self.vocab_len)
...
class AddSubDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inp = self.input_seqs[idx] # shape (K,)
        out = self.target_seqs[idx] # shape (O,)
        preprocess_inp = torch.tensor(self.preprocess(inp)) # shape (K,)
        preprocess_out = torch.tensor(self.preprocess(out)) # shape (O,)
        inp_pos = len(preprocess_inp) # shape ()
        inp_pos_enc = self.pos_encode(inp_pos, self.emb_dim) # shape (1, K, M)
        out_pos = len(preprocess_out) # shape ()
        out_pos_enc = self.pos_encode(out_pos, self.emb_dim) # shape (1, O, M)
