...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        for j in range(num_test):
            test_vec = x_test[j]
            cur_dist = torch.sum((train_vec - test_vec) ** 2)
            dists[i][j] = cur_dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    other_dims = len(x_test.shape)
    for i in range(num_train):
        train_vec = x_train[i]
        cur_dist = torch.sum((x_test - train_vec) ** 2, dim=tuple([i for i in range(1, other_dims)]))
        dists[i] = cur_dist.reshape(-1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_sq = torch.sum(x_train_flat ** 2, dim=1)
    x_test_sq = torch.sum(x_test_flat ** 2, dim=1)
    dists = x_train_sq[:, None] + x_test_sq.T[None, :] - 2 * (x_train_flat @ x_test_flat.T)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        train_col = dists[:, i]
        values, indices = torch.topk(train_col, k=k, dim=0, largest=False)
        neis = [j.item() for j in y_train[indices]]
        d = {}
        for nei in neis:
            if nei not in d:
                d[nei] = 1
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for fold in range(num_folds):
            ...
            train_X = torch.cat(tuple([x_train_folds[j] for j in range(num_folds) if j != fold]), dim=0)
            train_y = torch.cat(tuple([y_train_folds[j] for j in range(num_folds) if j != fold]), dim=0)
            cur_classifier = KnnClassifier(train_X, train_y)
            cur_acc = cur_classifier.check_accuracy(test_X, test_y, k, True)
            cur_accs.append(cur_acc)
        k_to_accuracies[k] = cur_accs
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        cur_mean_acc = torch.mean(torch.tensor(k_to_accuracies[k]))
        if not best_acc or best_acc < cur_mean_acc:
            best_k = k
            best_acc = cur_mean_acc
        elif best_acc == cur_mean_acc:
            best_k = min(best_k, k)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
