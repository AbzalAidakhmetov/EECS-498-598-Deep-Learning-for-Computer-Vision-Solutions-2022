...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for num in y:
        if num % 10 == 0.0:
            z.append(num)
    x = torch.tensor(z).to(torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    _, min_indecies = torch.min(y,  dim=1)
    y[torch.arange(x.shape[0]), min_indecies] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M, N = x.shape
    z = x.clone()
    sum_rows = torch.sum(z, dim=0)
    means = sum_rows / M
    stds = (torch.sum((z - means) ** 2, dim=0) / (N - 1)) ** 0.5
    y = (z - means) / stds
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    one_dim_tensor = torch.cat(xs)
    cum_sum = torch.cumsum(one_dim_tensor, dim=0)
    indecies_last = torch.cumsum(ls, dim=0) - 1
    sum_last = cum_sum[indecies_last]
    sum_first = torch.cat([torch.tensor([0]), sum_last[:-1]])
    overall_sum = sum_last - sum_first
    y = overall_sum / ls
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    uniques = sorted_values[first]
    indices = indecies[first]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
